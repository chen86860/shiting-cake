...
try {
    ...
    include "conn.php";
    $sql_orders = <<<cici
select * from cart where checked = 1 and userId = $userId
cici;
    $sql_orders_address = <<<cici
select * from userdata where id = $userId;
cici;
// order-goods
    mysqli_query($link, "set character set 'utf8'");
    $res_order_goods = mysqli_fetch_all(mysqli_query($link, $sql_orders), MYSQL_ASSOC);
    $res_order_address = mysqli_fetch_assoc(mysqli_query($link, $sql_orders_address));
    mysqli_close($link);
} catch (Exception $e) {
    echo $e->getMessage();
}
...
<div class="order-succed">
    ...
    <div class="order-wrap">
        <div class="address-h3 h3">
            <h3>收货人信息</h3>
            <p><?php echo $res_order_address['address'] ?></p>
        </div>
        <div class="address-h3 h3">
            <h3>支付方式</h3>
        ...
        <div class="shop-car-wrap">
            ...
            <div class="list-head-1">       
                    <a href=detail.php?id=' . $res_order_goods[$i]['goodId'] . ' class="list-good-img">
                        <img src="' . $res_order_goods[$i]['img'] . '" alt=""></a>
        ...
        <div class="shop-cls">
            <div class="statistic fr">
                <div class="list">
                <span><em class="ftx-01"><?php
                        echo sizeof($res_order_goods)
                        ?></em> 件商品，总商品金额：</span>
                    <em class="price" id="warePriceId" v="877.5">￥<?php
                        echo $good_total_mi
                        ?></em>
                </div>
                <div class="list">
                ...
                <div class="list">
                    <span>运费：</span>
                    <em class="price" id="freightPriceId"><i class="freight-icon"></i>
                        ￥0.00</font></em>
                </div>
            ...
            <div class="statistic">
                <p class="order-mon">应付金额: <span class="moneny">￥<?php
                        echo $good_total_mi
                        ?></span></p>
                <p>
                    <a class="subOrder">提交订单</a>
                </p>
            </div>
        </div>
